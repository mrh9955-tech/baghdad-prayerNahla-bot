...
# --- تهيئة تطبيق التليغرام و Flask معاً بنظام الـ Webhook ---
app = Flask(__name__)
application = Application.builder().token(TOKEN).build()

# إضافة المعالجات والـ JobQueue للتطبيق
...
# استقبال طلبات التحديث من تليغرام وتوجيهها للكود فوراً
@app.route(f'/{TOKEN}', methods=['POST'])
def webhook():
    if request.method == "POST":
        update = Update.de_json(request.get_json(force=True), application.bot)
        # تشغيل المعالجة بشكل غير متزامن داخل الـ Loop
        asyncio.run_coroutine_threadsafe(application.process_update(update), loop)
        return "OK", 200

@app.route('/')
def index():
    return "Prayer Bot Webhook Gateway Active"

# دالة لتهيئة البوت وربط الـ Webhook مع تليغرام عند الإقلاع
async def setup_webhook():
    await application.initialize()
    await application.start()
    # هنا يتم إجبار تليغرام على إرسال البيانات للرابط وحذف أي Polling قديم يسبب تعارض
    await application.bot.set_webhook(url=f"{WEBHOOK_URL}/{TOKEN}", drop_pending_updates=True)
    logging.info("Webhook set up successfully!")

if __name__ == '__main__':
    # إنشاء وتشغيل الـ Event Loop ليعمل بالتوازي مع Flask
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(setup_webhook())
    
    # تشغيل سيرفر Flask الرئيسي والمسؤول عن استقبال اتصالات Render وتليغرام
